...
const Projects = () => {
  ...
  const fetchTeams = async () => {
    try {
      const response = await axios.get('/api/teams', { params: { per_page: 200 } });
  ...
  const handleOpenDialog = (project = null) => {
    if (project) {
      ...
      const firstTeam = Array.isArray(project.teams) && project.teams.length > 0 ? project.teams[0] : null;
      setFormData({
        name: project.name,
        ...
        team_id: firstTeam ? firstTeam.id : '',
      });
    } else {
